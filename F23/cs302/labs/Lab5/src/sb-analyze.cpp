...
void Superball::analyze_superball(Disjoint_Set *d) {
  ...
  for (i = 0; i < r*c; i++) if (board[i] != '*') search(i, d);
...
void Superball::search(int index, Disjoint_Set *d) {
  ...
  if (j < r - 1 && board[index] == board[i*c + j + 1] && d->Find(index) != d->Find(i*c + j + 1)) d->Union(d->Find(index), d->Find(i*c + j + 1));
  return;
}
void Superball::store(Disjoint_Set *d) {
  ...
  sizes = d->Get_Sizes();
  for (i = 0; i < r*c; i++) {
    if (sizes->at(d->Find(i)) > 1 && goals[i]) idmap.insert(make_pair(d->Find(i), i));
  }
